  ...
  const { mutate: googleLoginMutation } = useMutate(
    ...
    ROUTE_PATH.AUTH.GOOGLE_LOGIN,
    {
      ...
      onError: (error) => {
        messageApi.open({
          type: 'error',
          content: error.response?.data?.message || 'Google login failed. Please try again.',
          duration: 2,
        });
      },
    }
      ...
      <AuthLayout>
        <div className="w-full max-w-lg ">
          ...
          <Form layout="vertical" form={form} onFinish={onFinish} requiredMark={false}>
            <Form.Item label="Email" name="email" rules={getValidationRule('email', true)} className="form-item">
              <Input placeholder="Enter your email" size="large" className="custom-placeholder" />
            </Form.Item>

            <Form.Item label="Password" name="password" rules={getValidationRule('password', true)} className="form-item">
            ...
            <div className="flex items-center justify-end mb-4 mt-[-6px]">
              <Link style={{ color: '#2363E3', fontWeight: '500' }} to={ROUTES.USER.FORGET_PASSWORD}>
                Forgot Password?
              </Link>
            </div>
            ...
            <Form.Item>
              <PrimaryButton htmlType="submit">
                {showLoader ? <LoadingButton size="small" /> : 'Sign in to RevenueSync'}
              </PrimaryButton>
            </Form.Item>
          </Form>
