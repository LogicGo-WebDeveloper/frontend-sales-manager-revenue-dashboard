    ...
    `${ROUTE_PATH.PAYMENT.DELETE}/${selectedDeleteAccountId}`,
    {
      onSuccess: async () => {
        queryClient.invalidateQueries({
          queryKey: [QUERY_KEYS.PAYMENT_METHOD.LIST],
        });
        await delay(1000);
        messageApi.open({
          type: "success",
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:hidden gap-3">
          <h2 className="text-sm font-semibold text-[#122751]">
            Payment Details
          </h2>
          <PrimaryButton
            onClick={onClickDrawer}
            // className="w-full h-[30px] text-[10px] "
            style={{ width: "100%", fontSize: "12px", height: "35px" }}
          >
            <FaPlus size={12} /> Add New Account
          </PrimaryButton>
        </div>
      ...
      <div className="text-[#122751]">
        <h3 className="sm:text-base text-sm font-medium mb-4">Bank Accounts</h3>

        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-6">
          {data?.body?.map((account, index) => (
            <div
              key={index}
              className="bg-white rounded-md border border-[#DCDFEA] p-6 w-full relative"
            >
              {/* Dropdown Menu Icon */}
              <Dropdown
                menu={getDropdownMenu(account)}
                trigger={["click"]}
              ...
              {/* Bank Details */}
              <div className="space-y-3 sm:text-base text-xs">
                <div>
                  <span className="text-[#8D94A3]">Account Name : </span>{" "}
                  {account.accountHolderName}
                </div>
                <div>
                  <span className="text-[#8D94A3]">Bank Name : </span>{" "}
                  {account.bankName}
                </div>
                <div>
                  <span className="text-[#8D94A3]">Branch Address : </span>{" "}
                  {account.bankAddress}
                </div>
                <div>
                  <span className="text-[#8D94A3]">Account Number : </span>{" "}
                  {account.accountNumber}
                </div>
                <div>
                  <span className="text-[#8D94A3]">IFSC Code : </span>{" "}
                  {account.ifscCode}
                </div>
                <div>
                  <span className="text-[#8D94A3]">SWIFT/BIC Code : </span>{" "}
      ...
      >
        <DeleteDrawer
          ...
          description="Are you certain you want to Delete Account? This action cannot be undone."
