...
import { Dropdown, Modal, Input, Form, Drawer, message as antdMessage } from 'antd';
...
const OpenTicketCard = () => {
    ...
    const { mutate: replyMutation, isPending } = useMutate(
        ...
        ROUTE_PATH.SUPPORT_TICKET.REPLY.replace(':ticketId', ticketId),
        {
            ...
            onError: (error) => {
                messageApi.error(error.response?.data?.message || 'Failed to send reply');
            }
        }
    );
    ...
    const { mutate: deleteReplyMutation, isPending: isDeletePending } = useMutate(
        ...
        ROUTE_PATH.SUPPORT_TICKET.DELETE_REPLY.replace(':ticketId', ticketId).replace(':replyId', selectedReplyId),
        {
            onSuccess: async () => {
                await delay(1000)
            ...
            onError: (error) => {
                messageApi.error(error.response?.data?.message || 'Failed to delete reply');
            }
        }
    );
    ...
    return (
        <>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md h-full flex flex-col justify-between gap-10" style={{ height: '100%', maxHeight: 'calc(100vh - 200px)' }}>
                <div className='overflow-auto pr-2'>
                    ...
                    {ticket.reply?.map((reply, index) => (
                        <div key={index} className="p-4 border-[#8d94a33f] border rounded-lg">
                            <div className="flex items-start">
                                <img src={profileImage} alt="" className="w-8 h-8 mt-1" />
                                <div className="flex-1 ml-3 mt-2 pr-6 border-b border-[#8d94a325] pb-2">
                                    <p className="font-semibold text-[#122751]">Reply from {reply.replyBy.email}</p>
                                    <p className="font-medium text-[#122751] text-sm mt-2">{ticket.title}</p>
                                    <p className="text-[#8D94A3] text-xs font-medium mt-1">{reply.description}</p>
                                </div>
                ...
                >
                    ...
                    >
                        ...
                        >
                            <TextArea rows={4} placeholder="Description..." style={{ resize: "none" }} />
                        </Form.Item>
                        ...
                        <Form.Item>
                            <div className="flex justify-end mt-4 gap-3">
                                ...
                                >
                                    {
                                        isPending ? <LoadingButton size='small' /> : "Submit Request"
                                    }
                                </PrimaryButton>
                            </div>
                        </Form.Item>
